...
bool ImGui::FileDialog(bool* open, ImFileDialogInfo* dialogInfo) {
  ...
  if (ImGui::Begin(dialogInfo->title.c_str(), open)) {
    ...
    ImGui::BeginChild("##browser",
                      ImVec2(ImGui::GetWindowContentRegionWidth(), 300), true,
    ...
    if (ImGui::Selectable("Name")) {
      ...
      fileNameSortOrder = fileNameSortOrder == ImGuiFileDialogSortOrder_Down
                              ? ImGuiFileDialogSortOrder_Up
                              : ImGuiFileDialogSortOrder_Down;
    }
    ImGui::NextColumn();
    ...
    if (dialogInfo->directoryPath.has_parent_path()) {
      if (ImGui::Selectable("..", dialogInfo->currentIndex == index,
    ...
    for (size_t i = 0; i < directories->size(); ++i) {
      ...
      if (ImGui::Selectable(directoryName.string().c_str(),
                            dialogInfo->currentIndex == index,
                            ImGuiSelectableFlags_AllowDoubleClick,
                            ImVec2(ImGui::GetWindowContentRegionWidth(), 0))) {
        dialogInfo->currentIndex = index;
